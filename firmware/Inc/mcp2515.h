...
#define SPI_DESELECT() HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET)
/*
 *  Speed 8M
 */
...
eERROR reset(void);
eERROR setConfigMode(void);
eERROR setListenOnlyMode(void);
eERROR setSleepMode(void);
eERROR setLoopbackMode(void);
eERROR setNormalMode(void);
eERROR setClkOut(const eCAN_CLKOUT divisor);
eERROR setBitrate(const eCAN_SPEED canSpeed, const eCAN_CLOCK canClock);
eERROR setFilterMask(const eMASK num, const bool ext, const uint32_t ulData);
